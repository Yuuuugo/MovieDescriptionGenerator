...
def process_data(file):
    ...
    data["year"] = data["prompt"].str.extract('(\d{4})', expand=True) # extract the year
    data['prompt'] = data['prompt'].map(lambda x: x.split("(")[0]) # delete the name
    data['completion'] = data['prompt'].map(lambda x: get_plot(x)) # get the plot
